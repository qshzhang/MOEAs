...
{
    public class WFG8_M : WFG.WFG
    {
        private static WFG8_M instance;

        private WFG8_M(int M) : base(M)
        {
            S_ = new int[M_];
            for (int i = 0; i < M_; i++)
                S_[i] = 2 * (i + 1);
        ...
        public static WFG8_M GetInstance(int M)
        {
            if (instance == null)
            {
                instance = new WFG8_M(M);
                instance.name = "WFG8_" + M;
            }
            return instance;
        }
        ...
        public double[] t1(double[] z, int k)
        {
            ...
            for (int i = k; i < z.Length; i++)
            {
                ...
                result[i] = (new WFG.Transformations()).b_param(z[i], aux, (double)0.98 / (double)49.98, (double)0.02, 50);
            }
